...
const byte STATIC_IP_BYTES[] = {192, 168, 10, 210};
const byte GATEWAY_BYTES[]   = {192, 168, 10, 1};
const byte SUBNET_BYTES[]    = {255, 255, 255, 0};
const byte PRIMARY_DNS_BYTES[]      = {8, 8, 8, 8};
const byte SECONDARY_DNS_BYTES[]      = {8, 8, 4, 4};
...
void handleConnection() {
    ...
    if (WiFi.status() == WL_CONNECTED && !State::system.ntpInitialized) {
        ...
        // ★★★ NTP同期ロジックを強化 ★★★
        Serial.print("Attempting to sync time with NTP server");
        
        // ★★★ 修正: NTP.stop() は存在しないため削除 ★★★
...
void init() {
    ...
    if (USE_STATIC_IP) {
        ...
        // ★★★ デバッグログを追加 & DNS設定を修正 ★★★
        // WiFi.configを呼び出す前に設定情報をシリアルモニタに出力します
